...
strategies = {
    ...
    "single_domain_double_shot": (
        setupSqlScript
        + "\nExample Q: Who doesn't have a way for us to text them?\n"
        + "Example A:\nSELECT p.person_id, p.name FROM person p LEFT JOIN phone ph ON p.person_id = ph.person_id AND ph.can_recieve_sms = 1 WHERE ph.phone_id IS NULL;\n"
        + common_sql_prompt
    ),
}
...
questions = [
    ...
    "Who was the commander in charge the longest?",
